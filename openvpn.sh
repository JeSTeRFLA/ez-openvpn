...
## Link to EasyRSA via GitHub ##
easy_rsa_url='https://github.com/OpenVPN/easy-rsa/releases/download/v3.1.2/EasyRSA-3.1.2.tgz'
## Link to icanhazip for public ip ##
get_public_ip=$(grep -m 1 -oE '^[0-9]{1,3}(\.[0-9]{1,3}){3}$' <<< "$(wget -T 10 -t 1 -4qO- "http://icanhazip.com/" || curl -m 10 -4Ls "http://icanhazip.com/")")
...
LimitNPROC=infinity" > /etc/systemd/system/openvpn-server@server.service.d/disable-limitnproc.conf
	...
	## Create the "Public key infrastructure", set up the CA and the server and client certificates ##
	./easyrsa init-pki
